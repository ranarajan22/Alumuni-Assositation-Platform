...
function StudentReg() {
  ...
  const submitForm = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8083/auth/signup', formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem('userId', response.data._id); // Assuming _id is the userId
    ...
    <>
      <div className="container mx-auto py-16 font-outfit">
        ...
        <form onSubmit={submitForm} className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
          ...
          <div className="flex flex-col">
            <label htmlFor="usn" className="text-gray-600 mb-2">Regestration Number</label>
            <input
              type="text"
              id="usn"
              name="usn"
              ...
              placeholder="Enter your Regestration Number"
              className="border rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
